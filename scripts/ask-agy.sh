...
EXPECTED_CMD_PATTERN='(^|[ /])(agy|omx(\.js)?|codex)([[:space:]]|$)'
...
if ! pgrep -a -P "$PANE_PID" | grep -Eq "$EXPECTED_CMD_PATTERN"; then
  ...
  echo "Refusing to send prompt: target pane $PANE_ID is running '$ACTUAL_CMD' (shell reported '$PANE_COMMAND'), not agy/omx/codex."
